...
// Return a pointer to the iotdb session
func NewSession(config db.Configuration, lc logger.LoggingClient) (*Session, error) {
	once.Do(func() {
		endPoint := endPoint{}
		endPoint.Host = config.Host
		endPoint.Port = strconv.Itoa(config.Port)
...
// CloseSession closes the connections to iotdb
